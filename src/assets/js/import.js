...
function VerificarUsuarioLogado() {
  ...
  const link = document.getElementById("link-favorito") || "Não encontrado...";

  const token = localStorage.getItem("token") || "Não encontrado...";

  if (userLogado && token) {
    const { nome } = userLogado;
    ...
    const link = document.getElementById("link-favoritos");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("/src/components/rodape.html")
    .then((response) => {
